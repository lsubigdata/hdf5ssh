...
main(int argc, const char *argv[])
{
    ...
    while(argno < argc) {
        ...
        if (!sshtag)
        {
            ...
            file = H5Fopen(fname, H5F_ACC_RDONLY, plist_id);

            if (file >= 0)
            {
                PRINTSTREAM(rawoutstream, "Opened \"%s\" with ssh remote access.\n", fname);
            }
        }
        else
        {
            while(fname && *fname) {
                //hid_t plist_id = H5Pcreate(H5P_FILE_ACCESS);
                //H5Fopenssh2(plist_id);
                //file = h5tools_fopen(fname, H5F_ACC_RDONLY, plist_id, preferred_driver, drivername, sizeof drivername);
                file = h5tools_fopen(fname, H5F_ACC_RDONLY, H5P_DEFAULT, preferred_driver, drivername, sizeof drivername);
                //file = H5Fopen(fname, H5F_ACC_RDONLY, plist_id);

                ///////////debug
                printf("fname = %s\n", fname);
                printf("preferred_driver = %s\n", preferred_driver);
                printf("drivername = %s\n", drivername);
                ////////////////
