...
static char  *H5PL_env_strdup(const char *s);
...
static char ExecutableDir[MAX_PATH+1] = "";
static size_t ExecPathLength = 0;

static char *H5PL_env_strdup(const char *s)
{
	if (!s)	return NULL;
	if (*s == H5PL_PLUGIN_ENV_SPECIALCHAR)
	{
		if (s[1] == '0') /* char sequence: @0 for main executable directory */
		{
			if (!*ExecutableDir)
			{
...
#endif 
				{
				char * bs = strrchr(result,  PathSep);
				size_t	ncopy = MAX_PATH;	
					if (bs)
						ncopy = bs - result;

					HDstrncpy(ExecutableDir, result, ncopy);
					ExecutableDir[MAX_PATH] = '\0';
				}
				ExecPathLength = strlen( ExecutableDir );
			} 

			{
			char*result = (char*)malloc(MAX_PATH+1);
				if (!result) /* out of memory problem */
					return result;

				HDstrncpy(result, ExecutableDir, ExecPathLength);
				HDstrncpy(result+ExecPathLength, s+2, MAX_PATH-ExecPathLength);
				result[MAX_PATH] = '\0';
				return result;
			}
		}
		...
		{
		/* TODO: handle other environment variables */
		}
	}
	return HDstrdup(s);
}
...
H5PL__init_path_table(void)
{
    ...
    if(NULL == dl_path)
     ...
     /*  printf("\n\n HDF5 Looking for plugins from: %s\n\n", dl_path); */
    ...
    while(dir) {
        ...
        if(NULL == (H5PL_path_table_g[H5PL_num_paths_g] = H5PL_env_strdup(dir)))
            HGOTO_ERROR(H5E_PLUGIN, H5E_CANTALLOC, FAIL, "can't allocate memory for path")
        H5PL_num_paths_g++;
